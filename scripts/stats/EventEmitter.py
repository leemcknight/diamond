...
class EventEmitter:
    ...
    def emit_blocked_pitch(self, blocked_pitch, game_state, box_score):
        print('blocked pitch'.format(blocked_pitch))
    ...
    def emit_pitch(self, pitch, game_state, box_score):
        ...
        pitching_line = box_score.get_pitching_line(
            pitcher_index).current_reliever()
    ...
    def emit_advance(self, advance, game_state, box_score):
        if ('X' in advance):
            ...
            if (end_base == 'H'):
                self.emit_run(runner, game_state, box_score)
                print("{} scores.".format(runner))
            else:
                game_state['runners'][end_base] = runner
                print("{} advances from {} to {}".format(
                    runner, start_base, end_base))

    def emit_modifier(self, mod, game_state):
        s = modifier(mod)
        print(s)
    ...
    def next_inning(self, game_state):
        ...
        game_state['runners'] = {}
        print('----------------------')
        print('We move to the {} of the {}.'.format(
            top_bottom, game_state['inning']))
    ...
    def emit_run(self, scorer, game_state, box_score):
    ...
    def parse_outs(self, play, game_state, box_score):
        ...
        if play.startswith('FC'):
            ...
            print('batter reaches on a fielders choice.  Out made by {}'.format(
                self.positions.get_player_and_position(pos, game_state)))
        else:
            unassisted = True
            last = ""
            next_is_runner_out = False
            print("out on a play from {}".format(
                self.positions.get_player_and_position(play[0], game_state)), end="")
            for idx in range(0, len(play)):
                ...
                if (c == "("):
                    ...
                    next_is_runner_out = False
                    fielder = self.positions.get_player_and_position(
                        play[idx], game_state)
                    if idx > 0:
                        unassisted = False
    ...
    def emit_play(self, play, game_state, box_score):
        ...
        print("{} {} out {} ".format(
            game_state['inning'], game_state['outs'], game_state['batter']), end="")
        pitching_team = '1' if game_state['inning'][0] == 'T' else '0'
        ...
        if (play.startswith('SB')):
            ...
            print('{} doubles to {}'.format(
                game_state['batter'], self.positions.get_player_and_position(play[1:2], game_state)))
            game_state['runners']['2'] = game_state['batter']
    ...
    def emit_events(self, event_data, game_state, box_score):
        ...
        if len(plays_modifiers_and_advances) > 1:
            ...
            for advance in advances:
                self.emit_advance(advance, game_state, box_score)
